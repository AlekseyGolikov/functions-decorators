...
# Очевидно, что выполнение complex_calculation не заняло времени. 
# Помните, что @нотация - это просто эквивалент записи MyClass = timer(MyClass), 
# т.е. декоратор будет вызван только тогда, когда вы «вызовете» класс. 
# Вызов класса означает создание его экземпляра, поэтому таймер отработает 
